...
const Post = ({ post, reaction, user, userProfile }) => {
  ...
  const handleChangeLike = (e) => {
    if (e.target.checked === true) {
      setIsLiked(true);
      dispatch(
        createLike(user.user_id, { post: { postId: Number(post.postId) } })
      );
    } else {
      setIsLiked(false);
  ...
  const handleSubmitAudience = (e) => {
    e.preventDefault();
    if (post.permission === audienceValue) {
      return null;
    } else {
  ...
  useEffect(() => {
    if (
      reaction.filter(
        (react) =>
          react.userId.user_id === user.user_id &&
          react.post.postId === post.postId
      ).length !== 0
    ) {
      setIsLiked(true);
    } else {
          ...
          user?.user_id === post?.user.user_id && (
            <>
                ...
                >
                  <ButtonBase onClick={() => handleClickOpen(2)}>
                    <Box display="flex" flexDirection="row" gap="5px">
                      <PublicIcon />
                      <Typography variant="text.secondary" fontWeight="bold">
                        Edit audience
                      </Typography>
                    </Box>
                  </ButtonBase>
              ...
              <EditAudience
                ...
                handleSubmitAudience={handleSubmitAudience}
              />
              {/* Open Edit Post*/}
                      ...
                      >
                        ...
                        >
                          ...
                          {post.permission === "Friends" && (
                            <img alt="icon" src={Friends} width="20%" />
                          )}
                          {post.permission === "Friends except" && (
                            <img alt="icon" src={FriendsExcept} width="20%" />
                          )}
                          {post.permission === "Only me" && (
                            <img alt="icon" src={Lock} width="20%" />
                          )}
              ...
              <Dialog open={openRecycleBin} onClose={() => handleClose(3)}>
                ...
                <DialogContent>
                  <Typography fontSize="14px">
                    Items in your recycle bin will be automatically deleted
                    after 30 days. You can delete them from your recycle bin
                    earlier by going to Activity log in your settings.
                  </Typography>
                </DialogContent>
                <DialogActions>
              ...
              >
                <div
                  style={{
                    ...
                    height: 30,
                    background:
                      "linear-gradient(180deg,rgba(66,103,178) 0%,rgba(33, 92, 255) 80%, rgba(54,90,189) 100%)",
                    opacity: 0.9,
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
