...
const AddPost = ({ user, userProfile }) => {
  ...
  const openProfile = () => {
    navigate(`/profile/${userProfile?.user_id || user?.result.sub}`);
  };
    ...
    >
      <Box display="flex" flexDirection="row">
        ...
        <div>
              ...
              <Box display="flex" flexDirection="column" sx={{ gap: "20px" }}>
                <Box display="flex" flexDirection="row" sx={{ gap: "20px" }}>
                  ...
                  >
                    ...
                    >
                      ...
                      {audienceValue === "Friends except" && (
                        <img alt="icon" src={FriendsExcept} width="20%" />
                      )}
                      {audienceValue === "Only me" && (
                ...
                {isShowImage && (
                  ...
                  >
                    ...
                    {!img && <FileBase multiple={false} onDone={getFiles} />}
                    {img && (
                      <img
                        alt="postImage"
                        src={img["base64"]}
          ...
          <EditAudience
            openAudience={openPermission}
            onClose={() => handleClose(2)}
            audienceValue={audienceValue}
            handleChangeAudience={handleChangeAudience}
            handleSubmitAudience={handleSubmitAudience}
            isFriendExcept={isFriendExcept}
            handleFriendExcept={handleFriendExcept}
          />
          {/** Open Friend Except */}
          <EditFriendExcept
            user={user}
            openFriendExcept={openFriendExcept}
